...
```bash
docker run --rm curder/laravel-docker:8.5.1 php -v
```
...
- GitHub Mirror
    ...
    ```bash
    docker run --rm ghcr.io/curder/laravel-docker:8.5.1 php -v
    ```

- Ali Mirror

    ```bash
    docker run --rm registry.cn-hangzhou.aliyuncs.com/curder/laravel-docker:8.5.1 php -v
    ```
