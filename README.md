...
| 8.4.24 | 8.4.24      | amd64/arm64  | ✅ Everything                                                                                             |
...
docker run --rm -v $(pwd):/var/www curder/laravel-docker:8.5.9 phpcs --standard=PSR12 src/

# Fix code style with Laravel Pint
docker run --rm -v $(pwd):/var/www curder/laravel-docker:8.5.9 pint src/
```

## Usage

```bash
docker run --rm curder/laravel-docker:8.5.9 php -v
```
...
- GitHub Mirror
    ...
    ```bash
    docker run --rm ghcr.io/curder/laravel-docker:8.5.9 php -v
    ```

- Ali Mirror

    ```bash
    docker run --rm registry.cn-hangzhou.aliyuncs.com/curder/laravel-docker:8.5.9 php -v
    ```
